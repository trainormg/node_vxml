...
module.exports = function(cfg, logger) {
  ...
  function speechToText(fileName, id, callback) {
    logger.profile('Nuance speechToText');
    ...
    var req = https.request(options, function(res) {
      logger.info('nuance call statusCode: ', res.statusCode);
...
//        console.log('end: %s', body);
        ...
        if (res.statusCode == 200) {
          ...
          logger.info("nuance stt, file: '%s', result: '%s'", fileName, result);
  
          if (callback) {
            callback(null, result);
          } else {
            logger.warn("nessuna callback, risultato ignorato");
          }
        } else if (callback) {
    ...
    req.on('error', function(err) {
      logger.error('error on Nuance http request ', err);
      if (callback) {
        callback(err);
      }
    });
